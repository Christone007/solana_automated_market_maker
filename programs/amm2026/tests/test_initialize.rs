...
fn test_initialize() {
    ...
    let counter = Pubkey::find_program_address(
        &[amm2026::constants::COUNTER_SEED],
        &program_id,
    )
    .0;
    ...
    let instruction = Instruction::new_with_bytes(
        program_id,
        &amm2026::instruction::Initialize {}.data(),
        amm2026::accounts::Initialize {
            payer: payer.pubkey(),
            counter,
    ...
    let counter_state = amm2026::state::Counter::try_deserialize(&mut data).unwrap();
    assert_eq!(counter_state.count, 0);
    assert_eq!(counter_state.authority, payer.pubkey());

    let instruction = Instruction::new_with_bytes(
        program_id,
        &amm2026::instruction::Increment {}.data(),
        amm2026::accounts::Increment {
            counter,
